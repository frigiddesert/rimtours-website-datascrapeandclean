...
def generate_markdown(tour, itinerary):
    ...
    # 4. Enhanced Fees Section (using ACF field data)
    fees_section = ""
    # Extract fees information from meta fields based on ACF definitions
    bike_fee = meta.get('Bike_Fee', meta.get('Fees_Bike', 'N/A'))
    camp_fee = meta.get('Camp_Fee', meta.get('Fees_Camp', 'N/A')) 
    shuttle_fee = meta.get('Shuttle_Fee', meta.get('Fees_Shuttle', 'N/A'))
...
"""
    ...
    # 5. Enhanced Description Section
    short_desc = meta.get('Short_Description', tour.get('Description_Short', ''))
    long_desc = meta.get('Long_Description', tour.get('Description_Long', ''))
    
    # 6. Additional Information Section
    additional_info = ""
    departs = meta.get('Departs', '')
    distance = meta.get('Distance', '')
    special_notes = meta.get('Special_Notes', '')
    dates = meta.get('Dates', '')
    
    if departs or distance or special_notes or dates:
        additional_info = "\n## 📋 Additional Information\n"
        if departs:
            additional_info += f"**Departs From:** {departs}\n\n"
        if distance:
            additional_info += f"**Distance:** {distance}\n\n"
        if dates:
            additional_info += f"**Available Dates:** {dates}\n\n"
        if special_notes:
            additional_info += f"**Special Notes:** {special_notes}\n\n"

    # 7. Pricing Section
    pricing_section = ""
    std_price = meta.get('Standard_Price', '')
    priv_price = meta.get('Private_Price', '')
    
    if std_price or priv_price:
        pricing_section = "\n## 💵 Pricing Information\n"
        if std_price:
            pricing_section += f"**Standard Price:** {std_price}\n\n"
        if priv_price:
            pricing_section += f"**Private Price:** {priv_price}\n\n"
...
## 1. The Shared DNA
**Subtitle:** {meta.get('Subtitle', tour.get('Meta', {}).get('Subtitle', ''))}  
**Region:** {meta.get('Region', tour.get('Meta', {}).get('Region', ''))}  
**Skill Level:** {meta.get('Skill', tour.get('Meta', {}).get('Skill', ''))}  
**Season:** {meta.get('Season', tour.get('Meta', {}).get('Season', ''))}
...
**Images (Filenames):**
`{meta.get('Gallery', tour.get('Meta', {}).get('Images', 'No images found'))}`

{pricing_section}{fees_section}{additional_info}
...
def main():
    ...
    except: return print("Run unify_tours_acf.py first!")
