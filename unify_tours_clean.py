...
def normalize_name(name):
    ...
    for k, v in MANUAL_OVERRIDES.items():
        if k in clean: return normalize_name(v)
...
def main():
    ...
    for idx, web in df_web.iterrows():
        ...
        # --- DATA MAPPING (Cleaned based on your feedback) ---
        meta = {
            # Marketing Text
            "Subtitle":     clean_html_text(get(['field_5633d6c4324ea', '_subtitle'])),
            "Region":       get(['field_56412af6db284', '_region', 'region', 'Region']), # E.g. "St. George"
            "Skill":        get(['_skill_level', 'skill_level']),
            "Season":       get(['_season', 'Season']),
            
            # Logistics
            "Distance":     get(['field_562fab0ca8d10', '_distance', 'distance']), # E.g. "80 miles"
            
            # Text Blocks (Cleaned of HTML)
            "Price_Notes":  clean_html_text(get(['field_562aba1626a27', 'standard_price', 'private_tour_price'])), # Solo/Group pricing text
            "Bike_Fee":     clean_html_text(get(['field_562fabffa8d13', '_bike_rental_fee', 'bike_rental_fee'])),
            "Camp_Fee":     clean_html_text(get(['field_5654e44ed46f1', '_camp_rental_fee', 'camp_rental_fee'])),
            "Shuttle_Fee":  clean_html_text(get(['field_562fac3ba8d16', '_shuttle_fee', 'shuttle_fee'])), # Sometimes hidden here?
            
            # Links & SEO
            "Book_Link":    get(['field_5a50fb5d4502f', 'reservation_link', 'Reservation Link']),
            "SEO_Title":    get(['_yoast_wpseo_title']),
            "SEO_Desc":     get(['_yoast_wpseo_metadesc']),
            
            # Images
            "Gallery":      extract_image_filenames(get(['Image URL', 'Featured Image', 'image_url'])),
            "Maps":         extract_map_images(get(['field_562fac3ba8d16', 'Content']))
        }
        ...
        record = {
            ...
            "Slug": web.get('Slug'),
            "Description_Short": clean_html_text(get(['Excerpt', 'short_description', 'field_562abaae555a3'])),
            "Description_Long": clean_html_text(get(['Content', 'description', 'field_562abad0555a4'])),
            "Arctic_Variants": [],
    ...
    print(f"Done. Processed {len(unified_list)} tours.")
