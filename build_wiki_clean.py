...
def extract_stats_from_text(text):
    """Extract Miles, Elev, Camp from raw text."""
...
def process_doc_content(doc):
    ...
    if not matches:
        # No day markers, return as single day
...
def generate_markdown(tour, itinerary):
    ...
    # 1. Get Primary Arctic Shortname (from first variant or list)
    ...
    # 4. Fee Block
    fee_section = ""
    if meta.get('Bike_Fee') or meta.get('Camp_Fee') or meta.get('Shuttle_Fee'):
        fee_section = f"""
## 💰 Fees & Logistics
| Item | Details |
| :--- | :--- |
| **Bike Rental** | {meta.get('Bike_Fee', 'N/A')} |
| **Camp Kit** | {meta.get('Camp_Fee', 'N/A')} |
| **Shuttle** | {meta.get('Shuttle_Fee', 'Included or N/A')} |
"""
...
def main():
    ...
    for tour in tours:
        ...
        # Fuzzy Search (both .doc and .docx)
        best_score = 0
        for fname in doc_db:
        ...
        if matched_doc_name:
            ...
            if category == "Day Tours":
                ...
                # Only multi-day tours without docs are "missing"
